...
function precalculateParty(members){
  return precalculateSupportParty(members, loadMemberCardLibrary());
}
...
function render(){
  ...
  currentActiveTimelineData = {
    songDuration: T, members: ms, eventsByMember: by, maxScore: max,
    passiveResults: party.passiveResults
  };
  const partial = ms.some(member => member.supportStatus === 'partial');
  document.querySelector('#supportEvaluation').textContent =
    (partial ? '未確定の対象選択・対象設定・重複補正は除外しています。評価値は確定できた補正のみです。 ' : '') +
    ms.map(member => `枠${member.slot}：基礎${member.baseBoost} + Support ${member.supportBoost.toFixed(2)} = ${member.effectiveBoost.toFixed(2)}%`).join(' ／ ');
